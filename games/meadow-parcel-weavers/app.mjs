...
function runPatternState() {
  ...
  if (ribbonArc && routeRut) return 'Dividend + Penalty';
  if (ribbonArc) return 'Dividend Ready';
  if (routeRut) return 'Penalty Risk';
  return 'Neutral';
}
...
function renderUI() {
  ...
  cartMultEl.textContent = `${getCartMultiplier(state).toFixed(2)}x`;
  ...
  if (state.phase === 'run') {
    ...
    patternStateEl.textContent = state.lastBreakdown.ribbonArc
      ? (state.lastBreakdown.routeRut ? 'Dividend + Penalty' : 'Dividend Applied')
      : (state.lastBreakdown.routeRut ? 'Penalty Applied' : 'Neutral');
    patternStateEl.style.color = state.lastBreakdown.ribbonArc ? '#2f9e71' : '#7d624d';
  }
...
function drawArena() {
  ...
  grad.addColorStop(1, '#ffedd7');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, w, h);

  const laneWidth = w / CONFIG.laneCount;
  for (let i = 0; i < CONFIG.laneCount; i += 1) {
    ctx.fillStyle = i % 2 === 0 ? 'rgba(236, 203, 166, 0.30)' : 'rgba(236, 203, 166, 0.16)';
