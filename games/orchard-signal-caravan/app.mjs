...
const signalStateEl = $('signalState');
...
function contractLabel(key) {
  return CONFIG.contracts[key]?.label || 'Village';
}

function runSignalState() {
  const h = state.runTierHistory || [];

  let chain = false;
  for (let i = 0; i <= h.length - 3; i += 1) {
    if (h[i] === 1 && h[i + 1] === 2 && h[i + 2] === 3) {
      chain = true;
      break;
    }
  }
  ...
  if (chain && monotony) return 'Chain + Penalty';
  if (chain) return 'Chain Ready';
  if (monotony) return 'Penalty Risk';
  return 'Neutral';
}
...
function renderUI() {
  ...
  if (state.phase === 'run') {
    signalStateEl.textContent = runSignalState();
    signalStateEl.style.color = '#c76a15';
  } else {
    signalStateEl.textContent = state.lastBreakdown.signalChain
      ? (state.lastBreakdown.monotonyPenalty ? 'Chain + Penalty' : 'Chain Applied')
      : (state.lastBreakdown.monotonyPenalty ? 'Penalty Applied' : 'Neutral');
    signalStateEl.style.color = state.lastBreakdown.signalChain ? '#2f9e71' : '#6e5b4d';
  }
...
function drawArena() {
  ...
  for (let i = 0; i < CONFIG.laneCount; i += 1) {
    ctx.fillStyle = i % 2 === 0 ? 'rgba(236, 196, 150, 0.30)' : 'rgba(236, 196, 150, 0.16)';
    ctx.fillRect(i * laneWidth, 0, laneWidth, h);

    ctx.strokeStyle = 'rgba(148, 104, 67, 0.22)';
    ctx.beginPath();
  ...
  for (const token of state.tokens) {
    ...
    const palette = ['#ffd89c', '#ffc36f', '#ffae4d', '#ff9731', '#ef7f1d'];
...
btnVillage.addEventListener('click', () => apply(chooseContract(state, 'village')));
btnGuild.addEventListener('click', () => apply(chooseContract(state, 'guild')));
btnRoyal.addEventListener('click', () => apply(chooseContract(state, 'royal')));
