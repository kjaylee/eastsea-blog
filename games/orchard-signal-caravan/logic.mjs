export const STORAGE_KEY = 'orchard_signal_caravan_save_v1';
...
export const CONFIG = {
  ...
  contracts: {
    village: { mult: 1, costCoins: 0, costGems: 0, label: 'Village' },
    guild: { mult: 1.3, costCoins: 60, costGems: 0, label: 'Guild' },
    royal: { mult: 1.56, costCoins: 0, costGems: 3, label: 'Royal' },
  },

  signalChainBonusMult: 1.24,
  monotonyPenaltyMult: 0.79,
  crashMult: 0.58,
};
...
function getContract(state) {
  return CONFIG.contracts[state.contract] || CONFIG.contracts.village;
}

function hasSignalChain(history) {
  for (let i = 0; i <= history.length - 3; i += 1) {
    if (history[i] === 1 && history[i + 1] === 2 && history[i + 2] === 3) {
      return true;
    }
  }
  return false;
...
export function createInitialState(saved = {}) {
  return {
    ...
    lastBreakdown: {
      ...
      signalChain: false,
      monotonyPenalty: false,
      chainMultiplier: 1,
      crashMultiplier: 1,
    },

    outcome: 'Orchard dock ready. Merge crates, reserve contract, then launch.',
  };
}
...
export function startRun(state) {
  ...
  next.outcome = `Run live (${getContract(next).label} contract). Collect crates, dodge wolves.`;
  return next;
}
...
function spawnToken(state, rng) {
  ...
  if (hazardRoll < 0.31) {
    state.tokens.push({ lane, y: -0.08, kind: 'wolf', tier: 0 });
    return;
  }
...
export function withInjectedTokens(state, tokens) {
  ...
  for (const token of tokens || []) {
    next.tokens.push({
      ...
      kind: token.kind === 'wolf' ? 'wolf' : 'crate',
...
function applyCollision(state, token) {
  if (token.kind === 'wolf') {
    ...
    state.runHits += 1;
    state.outcome = 'Wolf blocked the lane! Cart integrity down.';
    return;
  }
  ...
  state.outcome = `Secured Orchard Crate T${tier}.`;
}
...
export function settleRun(state, crashed = false) {
  ...
  const signalChain = hasSignalChain(next.runTierHistory);
  const monotonyPenalty = hasMonotony(next.runTierHistory);

  const chainMultiplier =
    (signalChain ? CONFIG.signalChainBonusMult : 1) *
    (monotonyPenalty ? CONFIG.monotonyPenaltyMult : 1);
  const crashMultiplier = crashed ? CONFIG.crashMult : 1;

  const payout = Math.max(
    0,
    Math.round(next.runValue * deckMultiplier * contractMultiplier * chainMultiplier * crashMultiplier),
  );

  const gemBonus = Math.floor(next.runHarvestCount / 6) + (signalChain ? 1 : 0) + (crashed ? 0 : 1);
  ...
  next.lastBreakdown = {
    ...
    signalChain,
    monotonyPenalty,
    chainMultiplier: Number(chainMultiplier.toFixed(3)),
    crashMultiplier,
  };
  ...
  next.spawnBankMs = 0;
  next.day += 1;
  next.contract = 'village';

  next.outcome = crashed
    ? `Run crashed. Salvage payout ${payout} coins.`
    : `Run clear. Payout +${payout}, gems +${gemBonus}${signalChain ? ' (Signal Chain!)' : ''}${
        monotonyPenalty ? ' (Monotony penalty)' : ''
      }.`;
...
export function mergeBestPair(state) {
  ...
  next.outcome = 'No mergeable crate pairs.';
  return next;
}
