...
### 1. 빌드 스크립트
**파일**: `scripts/update-novels.sh`

`_data/` 디렉토리의 모든 `.md` 파일을 스캔하여 `index.html`, `series.html`, `manifest.json`을 재생성합니다.
...
### 새 소설 추가
1. 첫 에피소드 파일 추가: `novels/_data/새소설슬러그-001.md`
   - Frontmatter에 최소 `title`, `episode`, `date`, `series`, `slug`, `author` 포함

2. 인덱스/시리즈/manifest 재생성
   ```bash
   AUTO_COMMIT=false ./scripts/update-novels.sh
   ```
