(function (global, factory) {
  ...
  function buildSummary(result, lang) {
    if (lang === 'ko') {
      return [
        '[Teachers Pay Teachers Seller Fee Calculator 요약]',
    ...
    return [
      '[Teachers Pay Teachers Seller Fee Calculator Summary]',
  ...
  const I18N = {
    en: {
      ...
      note: 'Planning posture only. This models one resource per order, not every possible cart mix, and it is not tax or accounting advice.',
      inputCopy: 'Adjust price, discount, and monthly sales to estimate how much TPT keeps and when Premium pays back.',
      assumptions: 'Assumption: Premium’s $0.15 fee applies only when the modeled order total is under $3. The Basic $29 fee is treated as sunk in the upgrade scenario.',
    ...
    ko: {
      ...
      note: '계획용 모델입니다. 여러 상품이 담긴 실제 장바구니를 완벽히 재현하지 않으며, 세무·회계 조언도 아닙니다.',
      inputCopy: '가격, 할인율, 월 판매량을 조정해 TPT가 가져가는 몫과 Premium 회수 시점을 빠르게 계산하세요.',
      assumptions: '가정: Premium의 $0.15 수수료는 주문 총액이 $3 미만일 때만 적용됩니다. 업그레이드 시나리오에서는 Basic $29를 매몰비용으로 처리합니다.',
      listPrice: '정가',
      discountPct: '할인율 (%)',
      monthlySales: '월 판매 수',
      reset: '기본값 복원',
